...
program.command('vnc')
    .description('Open VNC connection to the production Antigravity browser')
    .option('--host <host>', 'VNC host', 'halvarm')
    .option('--port <port>', 'VNC port', '5901')
    .option('--viewer <path>', 'Custom VNC viewer command')
    .action(async (options: { host: string; port: string; viewer?: string }) => {
        const { execSync } = await import('child_process');
        const target = `${options.host}:${options.port}`;

        console.log(`📡 Connecting to Antigravity Browser VNC at ${target}...`);

        // List of common viewers to try
        const viewers = options.viewer ? [options.viewer] : ['vncviewer', 'tigervnc', 'gvncviewer', 'remote-viewer'];
        let activeViewer = '';
        ...
        try {
            ...
            child.unref();
            console.log('✅ Viewer launched. Happy automating!');
        } catch (error: any) {
